---
title: "Linux Privilege Escalation: Full Checklist"
sidebarTitle: "Linux full checklist"
description: "Browse the saved upstream checklist for credentials, sudo, SUID, capabilities, scheduled tasks, writable files, groups, and kernel issues."
keywords: ["linux privesc", "sudo", "suid", "capabilities", "cron", "writable files", "gtfobins", "linpeas"]
---

> **Saved reference:** PayloadsAllTheThings / InternalAllTheThings (@swisskyrepo)
>
> **Upstream:** https://swisskyrepo.github.io/InternalAllTheThings/redteam/escalation/linux-privilege-escalation/
>
> **License:** MIT (see repo)
>
> **Tags:** [privesc, linux, checklist, cpts]

> **About this note**
> Mirrored from PayloadsAllTheThings' Linux Privilege Escalation page so it's available offline inside your Obsidian vault. Content is copyright its original authors (MIT licensed). If something looks outdated, always cross-check with the [upstream source](https://swisskyrepo.github.io/InternalAllTheThings/redteam/escalation/linux-privilege-escalation/) and with [HackTricks Linux PrivEsc Checklist](https://book.hacktricks.wiki/en/linux-hardening/linux-privilege-escalation-checklist.html).

> **How to use**
> 1. When you land a foothold, run `linpeas.sh` first (see §Tools).
> 2. Then walk the §Checklists section section-by-section.
> 3. When something looks interesting, jump to the deeper section below.
> 4. Always verify manually before trusting an automated tool's output.
...
* [Writable files](#writable-files)
    ...
    * [Writable /etc/sudoers](#writable-/etc/sudoers)
