---
title: "Windows Privilege Escalation: Full Checklist"
sidebarTitle: "Windows full checklist"
description: "Browse the saved upstream checklist for host configuration, credentials, services, task permissions, impersonation, and known escalation issues."
keywords: ["windows privesc", "services", "unquoted service path", "alwaysinstallelevated", "impersonation", "winpeas", "scheduled tasks", "uac"]
---

> **Saved reference:** PayloadsAllTheThings / InternalAllTheThings (@swisskyrepo)
>
> **Upstream:** https://swisskyrepo.github.io/InternalAllTheThings/redteam/escalation/windows-privilege-escalation/
>
> **License:** MIT (see repo)
>
> **Tags:** [privesc, windows, checklist, cpts]

> **About this note**
> Mirrored from PayloadsAllTheThings' Windows Privilege Escalation page so it's available offline inside your Obsidian vault. Content is copyright its original authors (MIT licensed). If something looks outdated, always cross-check with the [upstream source](https://swisskyrepo.github.io/InternalAllTheThings/redteam/escalation/windows-privilege-escalation/) and with [HackTricks Windows PrivEsc Checklist](https://book.hacktricks.wiki/en/windows-hardening/checklist-windows-privilege-escalation.html).

> **How to use**
> 1. After landing a foothold, run `winPEAS.exe` first (see §Tools) — or `Seatbelt.exe -group=all`.
> 2. Then walk each major section of this note in order: Version & Config → User/Network/AV enum → Looting passwords → Services → Unquoted paths → Impersonation privileges → Kernel/CVE.
> 3. `whoami /priv` + `whoami /groups` — 90% of Windows privesc on CPTS boxes is decided by these two commands.
> 4. If you see `SeImpersonatePrivilege` or `SeAssignPrimaryTokenPrivilege` → jump straight to §Impersonation Privileges (Potato family).
...
* [EoP - Common Vulnerabilities and Exposures](#eop-common-vulnerabilities-and-exposure)
    ...
    * [MS10-015 (KiTrap0D)](#ms10-015-kitrap0d-microsoft-windows-nt/2000/2003/2008/xp/vista/7)
    * [MS11-080 (adf.sys)](#ms11-080-afd-sys-microsoft-windows-xp/2003)
    * [MS15-051 (Client Copy Image)](#ms15-051-client-copy-image-microsoft-windows-2003/2008/7/8/2012)
    * [MS16-032](#ms16-032-microsoft-windows-7-<-10-/-2008-<-2012-r2-x86/x64)
