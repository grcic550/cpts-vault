...
sidebarTitle: "Privileged Remote Access"
description: "Find RDP, WinRM, and SQL access relationships and their lab examples."
keywords: ["RDP", "WinRM", "SQLAdmin", "CanRDP", "CanPSRemote", "remote access"]
---
...
[Scenario Setup](#scenario-setup) · [Remote Desktop](#remote-desktop) · [WinRM](#winrm) · [SQL Server Admin](#sql-server-admin) · [Moving On](#moving-on) · [Continue studying](#continue-studying)

{/* VAULT_QUICK_NAV_END */}

{/* Generated from Active Directory/Continued Lessons/18 Privileged Remote Access.md. Edit the Markdown source, then run scripts/sync_site.py. */}

**Lesson 18 of 28** · [AD study path](/active-directory/index)

**BloodHound CE:** use the [current startup guide](/tools/bloodhound) and [Cypher query library](/tools/bloodhound-cypher). The saved lesson below includes older interface and collector examples.
...
Graph showing DOMAIN USERS@INLANEFREIGHT.LOCAL with local admin and execution rights, connected to ACADEMY-EA-MS01.INLANEFREIGHT.LOCAL via CanRDP.
...
Node info for WLEY@INLANEFREIGHT.LOCAL showing execution rights, including Group Delegated RDP Privileges set to 1.
...
BloodHound graph showing connection from FOREND@INLANEFREIGHT.LOCAL to ACADEMY-EA-MS01.INLANEFREIGHT.LOCAL via CanPSRemote.
...
Interface for creating a custom query to find WinRM users with dangerous rights.
...
**Requested addition:** [BloodHound CE Cypher query library](/tools/bloodhound-cypher) — ACLs, ACEs, SPNs, membership, trusts, and sessions.
...
Graph showing SQLAdmin connection from ACADEMY-EA-DB01.INLANEFREIGHT.LOCAL to DAMUNDSEN@INLANEFREIGHT.LOCAL.

We can use our ACL rights to authenticate with the wley user, change the password for the damundsen user and then authenticate with the target using a tool such as PowerUpSQL, which has a handy command cheat sheet(https://github.com/NetSPI/PowerUpSQL/wiki/PowerUpSQL-Cheat-Sheet     usefull to look at this). Let's assume we changed the account password to SQL1234! using our ACL rights. We can now authenticate and run operating system commands.
...
[Previous: DCSync Lab](/active-directory/dcsync-lab) · [Next: Kerberos Double Hop](/active-directory/kerberos-double-hop)

[Preserved full capture](/downloads/active-directory-continuation-2026-09-09-d4996d60.txt). Source lines 785–1092. Illustration captions from the text export are retained.
